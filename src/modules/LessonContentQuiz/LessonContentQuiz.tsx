...
const exam = {
  test: {
    question_id: 2,
    question:
      "Выберите то, что не относится к методам обеспечения конфиденциальности",
    answers: [
      {
        answer_id: 2,
    ...
    <div class={styles.content}>
      ...
      <form class={styles.formAnswer}>
        {quiz.test.answers.map((block) => (
          <div
            class={`${styles.blockAnswer} ${
              right.answer_id != 0
                ? right.answer_id === block.answer_id
                  ? right.is_right === true
                    ? styles.right
                    : styles.wrong
                  : ""
          ...
          >
            <input
              type="radio"
              class={styles.blockAnswer__input}
              name="contact"
              {...(right.answer_id !== 0
                ? right.answer_id === block.answer_id
                  ? { checked: true }
