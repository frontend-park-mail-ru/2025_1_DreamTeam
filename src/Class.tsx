...
import { getAuthorizedUser, getLessons, getNextLessons, notCompleted } from "./api";
...
export function ClassText() {
  const [text, setText] = useState<CourseStructure>(exam);
  const [isLoading, setLoading] = useState(true);
  if (isLoading) {
    getLessons(1).then((result: CourseStructure) => {
        setText(result);
        setLoading(false);
    });
  }
  ...
  return (
    <div><ClassHeader key="ClassHeader" setText={setText} text={text} header={text.lesson.header} />
        <ClassContent key="ClassContent" setText={setText} text={text} />
    </div>
)
}


export function ClassHeader({setText, header, text} : {setText: (argv0: CourseStructure) => void, header: Header, text: CourseStructure}) {
    ...
    <div class="headlines under">{lesson}. {name_lesson} {lesson_after}. {name_lesson_after}</div>
    <div class="lesson--pages">
        <div class="headlines under">Пройдено шагов {count_watch.toString()} из {header.Points.length.toString()}</div>
        <div class="lessons--watch">
          {header.Points.map((lessons) => {
            ...
            <ButtonOnClick
              ...
              onClick={() => {getNextLessons(1, lessons.lesson_id).then((result) => {
                setText(result);
            })}}
            /> )
          })}
...
function ButtonOnClick({is_done, type, onClick, current_lesson_id, lesson_id}: {is_done: string, type: string, onClick: Function, current_lesson_id: number, lesson_id: number}) {
  if (current_lesson_id === lesson_id) {
    if (is_done === "true") {
        return (type === "video" ? <button class="page--lesson--choose" ON_click={onClick} style="border: 2px solid #E05C05"><img src="../static/icons/video.svg" alt="" class="video__icon" /></button> : <button class="page--lesson--choose" ON_click={onClick} style="border: 2px solid #E05C05"></button>
    )};
    return (type === "video" ? <button class="page--lesson--choose none" ON_click={onClick} style="border: 2px solid #E05C05"><img src="../static/icons/video_after.svg" alt="" class="video__icon" /></button> : <button class="page--lesson--choose none" ON_click={onClick} style="border: 2px solid #E05C05"></button>
    )
  };
  if (is_done === "true") {
    return (type === "video" ? <button class="page--lesson--choose" ON_click={onClick}><img src="../static/icons/video.svg" alt="" class="video__icon" /></button> : <button class="page--lesson--choose" ON_click={onClick}></button>
  )};
  return (type === "video" ? <button class="page--lesson--choose none" ON_click={onClick}><img src="../static/icons/video_after.svg" alt="" class="video__icon" /></button> : <button class="page--lesson--choose none" ON_click={onClick}></button>
  )
}

export function ClassContent({setText, text} : {setText: (argv0: CourseStructure) => void, text: CourseStructure}) {
  const body_lesson = text.lesson.lesson_body
  const ex = body_lesson.blocks.map((block) =>
      <div> {block.body} </div>
    );
  ...
  return (
    <div class="lesson--content">
        ...
        <PageButton
              ...
              page_id={pagesPrev}
              type="Предыдущая"
              onClick={() => {getNextLessons(1, pagesPrev).then((result) => {
                setText(result);
            })}}
            />
            <button class="page--check" ON_click={() => {notCompleted(body_lesson.footer.current_lesson_id)}}>
          ...
          <PageButton
            ...
            page_id={pagesNext}
            type="Следующая"
            onClick={() => {getNextLessons(1, pagesNext).then((result) => {
              setText(result);
          })}}
          />
        </div>
...
function PageButton({page_id, type, onClick}: {page_id: number, type: string, onClick: Function}) {
      return (page_id.toString() === "-1" ?
      <button class="page--choose" ON_click={onClick} disabled>
              <label>{type}</label></button> : <button class="page--choose" ON_click={onClick}>
              <label>{type}</label>
          </button>)
          }
